  ...
  <div class="container-sta">
    ...
    <div class="chart">
      <div class="chart1 chart-info">
        ...
        <h4>Đồ thị về số tài khoản đăng ký mới trong 5 tháng gần nhất</h4>
      </div>
      <div class="chart2 chart-info">
        <Chart class="detail-chart" :data="datatmpDoughnut" :type="typeDoughnut"/>
        <h4>Đồ thị về số tài khoản đăng ký gói</h4>
...
export default {
  ...
  setup() {
    ...
    const dataDoughnut = ref({
      labels: ['Chưa đăng ký', "Đã đăng ký"],
      datasets: [{
        label: 'Số tài khoản',
        data: [],
        backgroundColor: [
    ...
    const callAPI = async () => {
      try {
        const response = await axios.get("http://localhost:8080/account/getformonth");
        const response1 = await axios.get("http://localhost:8080/account/accountsub")
        ...
        //dataDoughnut
        accountSub.value = response1.data
        dataDoughnut.value.datasets[0].data[0] = accountSub.value[0] - accountSub.value[1]
        dataDoughnut.value.datasets[0].data[1] = accountSub.value[1]
